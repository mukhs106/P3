...
const CONFIG = {
  imageHeight: 180,
  driftSpeed: 10000, // Higher = slower
  cloudPadding: {
    top: 80,
    bottom: 220,
    left: 180,
...
function initCloud() {
  ...
  archiveData.forEach((imgData, index) => {
    const position = {
      ...
      z: CONFIG.zRange.min + Math.random() * (CONFIG.zRange.max - CONFIG.zRange.min),
...
function selectImage(id) {
  ...
  if (selectedImageId) {
    ...
    if (prevTile) {
      ...
      // Restore drift position
      const baseX = parseFloat(prevTile.dataset.baseX);
      const baseY = parseFloat(prevTile.dataset.baseY);
...
window.addEventListener('resize', () => {
  ...
  resizeTimeout = setTimeout(() => {
    ...
    document.querySelectorAll('.image-tile').forEach((tile, index) => {
      const currentX = parseFloat(tile.dataset.baseX);
      const oldWidth = bounds.width;

      // Proportionally adjust X position
      const newX = safeArea.minX + (currentX - CONFIG.cloudPadding.left) /
                   (oldWidth - CONFIG.cloudPadding.left - CONFIG.cloudPadding.right) *
                   (safeArea.maxX - safeArea.minX);

      tile.dataset.baseX = Math.max(safeArea.minX, Math.min(safeArea.maxX, newX));
    });
  }, 250);
});
